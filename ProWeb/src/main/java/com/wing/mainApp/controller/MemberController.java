...
public class MemberController {
	//	이 컨트롤러에서 사용할 DAO는 물론 필요한 시점에서 new시켜서 사용해도 된다.
	//	다만 스프링이 자랑ㅇ하는 기술중에 DI 기법이 있으므로
	...
	public ModelAndView loginProc(HttpServletRequest req, HttpSession session) {
		...
		// 이제 데이터 베이스에게 질의를 던져서 로그인을 해주도록 한다.
		//	1. SQL을 만든다
		//		com.uknow.dol.sql.MemberSQL.xml
		//	2. DAO를 이용해서 데이터베이스 처리를 한다.
		//		com.uknow.dol.dao.MemberDAO.java		
		...
		if(result == null || result.isEmpty()) {
			...
			//  결과물이 존재하니		로그인 성공
			//	세션에 그 결과물의 정보를 입력해 줌으로 나중에 세션을 이용해서
			//	이 사람의 정보를 얻을 수 있도록 해준다.
	...
	public ModelAndView loginOut(HttpSession session) {
		...
		// 참고
		//		원래는 이 부분에서 다른곳에서 사용했던 모든 세션을 같이 없에야 한다.
		session.invalidate();
